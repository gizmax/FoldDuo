...
class ManagedProfileIntegrationTest {
    ...
    @get:Rule val rules = org.junit.rules.RuleChain.outerRule(compose)
    ...
    @Test fun personalAndDisposableWorkCopiesRemainIndependentThroughQuietModeAndUninstall() {
        ...
        shell("am start --user $workUserId -a cz.pflanzer.foldduo.test.ENABLE_CROSS_PROFILE_WIDGETS " +
            "-n cz.pflanzer.foldduo.test/.ProfileFixtureActivity --es widgetPackage $removablePackage " +
        ...
        val fixture = ComponentName("cz.pflanzer.foldduo.test", "cz.pflanzer.foldduo.test.ProfileFixtureActivity")
        ...
        assertTrue(controller.providers(personal).any { it.provider.packageName == "cz.pflanzer.foldduo.test" })
