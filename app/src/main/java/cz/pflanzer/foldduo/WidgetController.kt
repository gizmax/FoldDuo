...
class WidgetController(
    ...
    fun remove(slot: Int) {
        ...
        // iOS behaviour: a page left empty by this removal disappears if it is trailing.
        model.trimTrailingEmptyPages()
        reconcileHostIds()
    }

    /** "Smazat stránku?" (2026-09-17 noc): force-deletes home page [page] — see
     * [LauncherModel.deleteHomePage]. Any host widget the page held is released through the same
     * reconciliation [remove] uses for a single widget, once undo is no longer keeping it retained. */
    fun removeHomePage(page: Int) {
        model.deleteHomePage(page)
        reconcileHostIds()
    }
    ...
    private fun legacyPlacement(slot: Int, id: Int): WidgetPlacement = if (slot % 3 == 2)
        // The "third slot" full-width overflow row is pinned at its historical row (GRID_ROWS was
        // 6 when this shape was fixed) — never the live GRID_ROWS, or a later grid change would
        // stop this matching [replaceWidgetAtSameFootprint]'s retained-legacy check.
        WidgetPlacement(slot, id, if (slot == 2) -1 else slot / 3, 0, if (slot == 2) 0 else LEGACY_FULL_WIDTH_OVERFLOW_ROW, 4, if (slot == 2) 6 else 4)
