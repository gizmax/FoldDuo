...
/**
 * 17. 9. reorg: this is now only the Wallpaper page's "Appearance" block — mode (light/dark/
 * follow system/sunrise-sunset) and, when the last of those is picked, the place/lat/lon fields.
 * Everything else that used to be appended after it (Continuum, Today, Colours & glass, Motion &
 * haptics rows) moved into settings/ pages of its own — same [CustomizationSheet], same rows, just
 * grouped by what they tune instead of one flat list. See CustomizationSheet.kt's OVERVIEW tiles.
 */
@Composable
...
@Composable
internal fun AppearanceFeedback(message: String?, color: androidx.compose.ui.graphics.Color, tag: String) {
