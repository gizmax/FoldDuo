...
data class AppEntry(
    ...
    /** `ApplicationInfo.category`; [ManifestCategory.UNDEFINED] when the app declares none. */
...
data class LauncherState(
    ...
    /** B46 "Dvojice aplikací" (PairEditing.kt has the pure editing rules). */
    ...
    /** Package names launched from this launcher, newest first (App Library "Suggestions"). */
    val recentLaunches: List<String> = emptyList(),
    /** Icon shape, glass effect and icon pack; defaults keep the original rounded-square system icons. */
    val iconStyle: IconStyle = IconStyle(),
    /** See [HomeLayout.explicitPageCount]. */
    val explicitPageCount: Int = 1,
    /** See [HomeLayout.hiddenPages] (B39 page overview checkmark). */
    val hiddenPages: Set<Int> = emptySet(),
    /** B39 "Režimy plochy": always contains at least the built-in [ALL_MODE_ID] "Vše" mode. */
    val homeModes: List<HomeMode> = defaultHomeModes(),
    /** The mode currently switched in (manual, scheduled, or DND); see [effectiveHiddenPages]. */
    val activeModeId: String = ALL_MODE_ID,
    /**
     * "Stránkování inneru jako spread" (17. 9. večer): true (default) pages the inner workspace as
     * one continuous [Today, P0, P1, …] strip, both panes advancing together one pane per swipe;
     * false keeps the previous behaviour, where only the right (Home) pane pages and Today never
     * moves. Home layout page setting, cover-only launcher state has no use for it.
     */
    val innerPagingSpread: Boolean = true,
) {
    val order: List<String> get() = homeSlots.filterNotNull()
    val widgets: List<Int> get() = layout.widgets
    val layout: HomeLayout get() = HomeLayout(homeSlots, dock, widgetPlacements, folders, widgetRestores, leadingSlots, explicitPageCount, hiddenPages, pairs)
    val homePages get() = layout.pageCount
    /** What the pager/indicator should actually skip right now: manual hides plus the active mode's filter. */
    val effectiveHiddenHomePages: Set<Int> get() = effectiveHiddenPages(homePages, hiddenPages, homeModes, activeModeId)
}

internal const val LAUNCH_HISTORY_LIMIT = 20
...
class LauncherModel(application: Application) : AndroidViewModel(application) {
    ...
    private val mutable = MutableStateFlow(load().copy(recentLaunches = loadLaunchHistory()))
    ...
    // Bumped to force every icon to re-render ("Refresh icons", icon pack updated).
    @Volatile private var iconRevision = 0
    // Accessed only in the serialized IO refresh.
    private var loadedPack: LoadedIconPack? = null
    internal var completedRefreshes = 0
        private set
    private val callback = object : LauncherApps.Callback() {
        override fun onPackageAdded(packageName: String, user: UserHandle) {
            if (packageName == mutable.value.iconStyle.pack) iconRevision++
            refresh(packageName, user)
        }
        override fun onPackageRemoved(packageName: String, user: UserHandle) {
            removedPackages += userManager.getSerialNumberForUser(user) to packageName
            if (packageName == mutable.value.iconStyle.pack) { setIconStyle(mutable.value.iconStyle.copy(pack = null)); return }
            refresh(packageName, user)
        }
    ...
    fun refresh(invalidatedPackage: String? = null, user: UserHandle = Process.myUserHandle()) {
        ...
        val iconStyle = mutable.value.iconStyle
        val revision = iconRevision
        val baseConfiguration = resources.configuration.let { "${it.densityDpi}|${it.locales.toLanguageTags()}|${it.uiMode}" }
        viewModelScope.launch {
            try {
                val apps = withContext(Dispatchers.IO) {
                    val packVersion = iconStyle.pack?.let { pkg -> runCatching {
                        getApplication<Application>().packageManager.getPackageInfo(pkg, 0).lastUpdateTime }.getOrNull() }
                    val configuration = "$baseConfiguration|${iconStyle.shape}|${iconStyle.effect}|${iconStyle.pack}@$packVersion|$revision"
                    if (configuration != iconConfiguration) {
                        iconCache.clear(); iconConfiguration = configuration
                        loadedPack = iconStyle.pack?.takeIf { packVersion != null }
                            ?.let { LoadedIconPack.load(getApplication(), it) }
                    }
                    val pack = loadedPack
                    fun render(drawable: Drawable, component: ComponentName) =
                        runCatching { IconRenderer.render(drawable, iconStyle, pack, component) }
                            .getOrElse { IconRenderer.shaped(drawable, iconStyle.shape) }
                    ...
                    val cachedBeforeProfiles = loadCachedApps(iconStyle).filterNot { entry -> entry.userSerial to entry.packageName in removed }
                    ...
                    val live = handles.flatMap { profile ->
                        ...
                        if (activityList == null) emptyList() else activityList.also { authoritativeProfiles += serial }.mapNotNull { info ->
                            ...
                            iconCache[id]?.takeIf { it.label == label && it.available } ?: run {
                                ...
                                val applicationInfo = runCatching { info.applicationInfo }.getOrNull()
                                AppEntry(id, label, render(icon, component), component, profile, serial, descriptor.label,
                                    descriptor.isWork, available = true,
                                    manifestCategory = applicationInfo?.category ?: ManifestCategory.UNDEFINED,
                                    isSystem = applicationInfo?.let { it.flags and android.content.pm.ApplicationInfo.FLAG_SYSTEM != 0 } ?: false,
                ...
                mutable.update { old ->
                    ...
                    val removedIds = removedAppIds(old.homeSlots.filterNotNull() + old.leadingSlots.filterNotNull() +
                        old.dock.filterNotNull() + old.folders.flatMap { it.appIds } + old.pairs.flatMap { listOf(it.first, it.second) },
                        availableIds, authoritative, temporarilyUnavailable, removed, userManager.getSerialNumberForUser(Process.myUserHandle()),
                    ...
                    val reconciled = reconcilePairs(reconcileFolders(HomeLayout(validPins, validDock, old.widgetPlacements, old.folders,
                        old.widgetRestores, old.leadingSlots, old.explicitPageCount, old.hiddenPages, old.pairs), removedIds), removedIds)
                    old.copy(apps = entries, profiles = profiles, homeSlots = reconciled.slots, leadingSlots = reconciled.leadingSlots,
                        dock = reconciled.dock, folders = reconciled.folders, pairs = reconciled.pairs, explicitPageCount = reconciled.explicitPageCount,
                        hiddenPages = reconciled.hiddenPages,
    ...
    private fun loadCachedApps(iconStyle: IconStyle): List<AppEntry> = runCatching {
        val application = getApplication<Application>()
        val personal = Process.myUserHandle()
        ...
        List(array.length()) { index ->
            ...
            // Unavailable apps show the generic activity icon in the chosen shape and effect (never pack artwork).
            AppEntry(id, item.getString("label"), IconRenderer.render(icon, iconStyle.copy(pack = null), null, null), component, user, serial,
                item.optString("profile", if (isWork) "Work" else "Personal"), isWork, available = false,
                manifestCategory = item.optInt("category", ManifestCategory.UNDEFINED), isSystem = item.optBoolean("system", false),
                firstInstallTime = item.optLong("installed", 0L))
        }
    ...
    private fun saveCachedApps(apps: List<AppEntry>) {
        val array = JSONArray().also { result -> apps.forEach { app -> result.put(JSONObject()
            ...
            .put("category", app.manifestCategory).put("system", app.isSystem).put("installed", app.firstInstallTime)) } }
        appCatalogPrefs.edit().putString("apps", array.toString()).apply()
    }

    /** Launcher-side launch history for App Library "Suggestions"; no usage-stats permission. */
    fun recordLaunch(app: AppEntry) {
        val history = (listOf(app.packageName) + mutable.value.recentLaunches).distinct().take(LAUNCH_HISTORY_LIMIT)
        mutable.update { it.copy(recentLaunches = history) }
        prefs.edit().putString("launch_history", JSONArray(history).toString()).apply()
        // B35 "Předpovědi aplikací": every launch also feeds the Today Suggestions row's own,
        // longer-lived event store (cz.pflanzer.foldduo.predict), capped and scored separately
        // from this plain history.
        cz.pflanzer.foldduo.predict.PredictionController(getApplication<Application>()).recordLaunch(app.packageName)
    }

    private fun loadLaunchHistory(): List<String> = runCatching {
        val array = JSONArray(prefs.getString("launch_history", "[]") ?: "[]")
        List(array.length()) { array.getString(it) }.filter { it.isNotBlank() }.distinct().take(LAUNCH_HISTORY_LIMIT)
    }.getOrDefault(emptyList())
    ...
    fun applyDrop(id: String, target: DropTarget): Boolean {
        ...
        val pair = old.layout.pair(id)
        if (old.apps.none { it.id == id } && folder == null && pair == null) return false
        if (target is DropTarget.Folder) return folder == null && pair == null && addAppToFolder(target.id, id)
        if ((folder != null || pair != null) && target !is DropTarget.Home) return false
    ...
    fun createFolder(firstAppId: String, secondAppId: String, targetIndex: Int, title: String = "Folder"): String? {
        ...
        val id = newFolderId()
        return id.takeIf { commitLayout(cz.pflanzer.foldduo.createFolder(mutable.value.layout, FolderEntry(id, title, emptyList()),
            firstAppId, secondAppId, targetIndex)) }
    }
    fun renameFolder(folderId: String, title: String) = commitLayout(cz.pflanzer.foldduo.renameFolder(mutable.value.layout, folderId, title))
    fun addAppToFolder(folderId: String, appId: String, index: Int? = null): Boolean {
        ...
        return commitLayout(cz.pflanzer.foldduo.addAppToFolder(mutable.value.layout, folderId, appId, index))
    }
    fun removeAppFromFolder(folderId: String, appId: String, target: DropTarget) =
        commitLayout(cz.pflanzer.foldduo.removeAppFromFolder(mutable.value.layout, folderId, appId, target))
    fun moveFolderApp(folderId: String, appId: String, index: Int) =
        commitLayout(cz.pflanzer.foldduo.moveFolderApp(mutable.value.layout, folderId, appId, index))
    fun folder(id: String) = mutable.value.layout.folder(id)

    // --- B46 "Dvojice aplikací" ------------------------------------------------------------

    fun createPair(firstAppId: String, secondAppId: String, targetIndex: Int): String? {
        val installed = mutable.value.apps.mapTo(mutableSetOf(), AppEntry::id)
        if (firstAppId !in installed || secondAppId !in installed) return null
        val id = newPairId()
        return id.takeIf { commitLayout(cz.pflanzer.foldduo.createPair(mutable.value.layout, PairEntry(id, firstAppId, secondAppId), targetIndex)) }
    }
    fun swapPairSides(pairId: String) = commitLayout(cz.pflanzer.foldduo.swapPairSides(mutable.value.layout, pairId))
    fun splitPair(pairId: String) = commitLayout(cz.pflanzer.foldduo.splitPair(mutable.value.layout, pairId))
    fun removePair(pairId: String) = commitLayout(cz.pflanzer.foldduo.removePair(mutable.value.layout, pairId))
    fun pair(id: String) = mutable.value.layout.pair(id)
    ...
    fun applyImportedLayout(preview: LayoutImportPreview): Boolean {
        ...
        mutable.value = old.copy(homeSlots = preview.layout.slots, leadingSlots = preview.layout.leadingSlots, dock = preview.layout.dock,
            ...
            widgetRestores = preview.layout.widgetRestores, explicitPageCount = preview.layout.explicitPageCount,
            hiddenPages = preview.layout.hiddenPages, pairs = preview.layout.pairs,
    ...
    fun setWidgetAppearance(slot: Int, appearance: WidgetAppearance, tint: Int?) =
        commitLayout(styleWidget(mutable.value.layout, slot, appearance, tint))
    // B26 Smart stack (WidgetStack.kt has the pure editing rules).
    fun stackWidgets(targetSlot: Int, draggedSlot: Int) = commitLayout(stackWidgets(mutable.value.layout, targetSlot, draggedSlot))
    fun removeStackMember(slot: Int, memberId: Int) = commitLayout(removeStackMember(mutable.value.layout, slot, memberId))
    fun reorderStackMember(slot: Int, from: Int, to: Int) = commitLayout(reorderStackMember(mutable.value.layout, slot, from, to))
    fun cycleStackMember(slot: Int, forward: Boolean) = commitLayout(cycleStackMember(mutable.value.layout, slot, forward))
    fun showStackMember(slot: Int, memberId: Int) = commitLayout(showStackMember(mutable.value.layout, slot, memberId))
    fun setStackSmartRotate(slot: Int, enabled: Boolean) = commitLayout(setStackSmartRotate(mutable.value.layout, slot, enabled))
    ...
    /** The "+" / "Přidat stránku" affordance: appends one empty page after the last. Covers every
     * call site (the trailing "+" dot and B39's PageOverview "+" button both call this one
     * function), so B36's PAGE_ADDED haptic only needs wiring here, once. */
    fun addHomePage() {
        commitLayout(addHomePage(mutable.value.layout))
        Haptics.play(getApplication(), HapticEvent.PAGE_ADDED)
    }
    /** "Odebrat stránku": removes an empty page, refusing (see [removeHomePage]) if it still holds content. */
    fun removeHomePage(page: Int) = commitLayout(removeHomePage(mutable.value.layout, page))
    /**
     * "Smazat stránku?" (2026-09-17 noc "Mazání stránek s dotazem"): deletes home page [page] even
     * when it still holds an icon, folder, pair or widget — see [removeHomePage]'s `force`
     * parameter for exactly what happens to each kind of occupant. Undoable like any other layout
     * edit through [commitLayout]. The caller (normally [WidgetController.removeHomePage]) is
     * responsible for releasing any host widget id the deleted placements held, same as a single
     * widget removal. Also remaps [page] out of every mode's visible-page set ([remapModesAfterPageRemoval])
     * — modes are not part of the undo/redo layout history (see "Režimy plochy" below), so that
     * cannot ride along inside [commitLayout] itself.
     */
    fun deleteHomePage(page: Int): Boolean {
        val old = mutable.value
        if (page !in 0 until old.homePages || old.homePages <= 1) return false
        val committed = commitLayout(removeHomePage(old.layout, page, force = true))
        if (committed) {
            mutable.update { it.copy(homeModes = remapModesAfterPageRemoval(it.homeModes, page)) }
            persist()
        }
        return committed
    }
    /** iOS behaviour: once an edit gesture settles, trailing empty pages disappear (middle ones stay). */
    fun trimTrailingEmptyPages() = commitLayout(trimTrailingEmptyPages(mutable.value.layout))
    /** B39 page overview: the drag-to-reorder gesture, committed once it settles. */
    fun reorderHomePages(from: Int, to: Int) = commitLayout(reorderHomePages(mutable.value.layout, from, to))
    /** B39 page overview: the checkmark that hides/shows a page from the pager and indicator. */
    fun setPageHidden(page: Int, hidden: Boolean) = commitLayout(setPageHidden(mutable.value.layout, page, hidden))
    private fun commitLayout(next: HomeLayout): Boolean {
        ...
        mutable.value = old.copy(homeSlots = next.slots, leadingSlots = next.leadingSlots, dock = next.dock,
            ...
            widgetRestores = next.widgetRestores, explicitPageCount = next.explicitPageCount,
            hiddenPages = next.hiddenPages, pairs = next.pairs,
            editRevision = old.editRevision + 1, canUndoEdit = true)
        persist()
        return true
    }

    // --- B39 "Režimy plochy": modes are their own list, not part of undo/redo layout history. ---

    /** Creates a mode with every current page visible and switches nothing; returns its new id. */
    fun createMode(name: String, icon: String = "apps"): String {
        val id = "mode-" + java.util.UUID.randomUUID().toString().take(8)
        val allPages = (0 until mutable.value.homePages).toSet()
        mutable.update { it.copy(homeModes = it.homeModes + HomeMode(id, name.ifBlank { "Mode" }, icon, allPages)) }
        persist()
        return id
    }
    fun renameMode(id: String, name: String) {
        if (id == ALL_MODE_ID || name.isBlank()) return
        mutable.update { it.copy(homeModes = it.homeModes.map { m -> if (m.id == id) m.copy(name = name) else m }) }
        persist()
    }
    /** Refuses the built-in "Vše" mode; falls back the active selection to it if the deleted mode was active. */
    fun deleteMode(id: String) {
        if (id == ALL_MODE_ID) return
        mutable.update { old -> old.copy(homeModes = old.homeModes.filterNot { it.id == id },
            activeModeId = if (old.activeModeId == id) ALL_MODE_ID else old.activeModeId) }
        persist()
    }
    fun setModeVisiblePages(id: String, pages: Set<Int>) {
        if (id == ALL_MODE_ID) return
        mutable.update { it.copy(homeModes = it.homeModes.map { m -> if (m.id == id) m.copy(visiblePages = pages) else m }) }
        persist()
    }
    fun setModeSchedule(id: String, schedule: ModeSchedule?) {
        if (id == ALL_MODE_ID) return
        mutable.update { it.copy(homeModes = it.homeModes.map { m -> if (m.id == id) m.copy(schedule = schedule) else m }) }
        persist()
    }
    /** Only one mode can be DND-flagged at a time; setting it clears any other. */
    fun setModeDndTrigger(id: String, enabled: Boolean) {
        if (id == ALL_MODE_ID) return
        mutable.update { it.copy(homeModes = it.homeModes.map { m -> m.copy(dndTrigger = if (m.id == id) enabled else if (enabled) false else m.dndTrigger) }) }
        persist()
    }
    /** Manual, scheduled, or DND trigger all funnel through here; logs under `FoldDuoModes` per the task. */
    fun switchMode(id: String, reason: String = "manual") {
        val mode = mutable.value.homeModes.firstOrNull { it.id == id } ?: return
        if (mutable.value.activeModeId == id) return
        android.util.Log.d("FoldDuoModes", "switch to ${mode.name} ($reason)")
        mutable.update { it.copy(activeModeId = id) }
        persist()
    }
    ...
    fun undoEdit(): Boolean {
        ...
        mutable.value = old.copy(homeSlots = reconcileHomeSlots(before.slots, installed),
            ...
            widgetRestores = before.widgetRestores, explicitPageCount = before.explicitPageCount, hiddenPages = before.hiddenPages,
            pairs = before.pairs,
            compact = settings?.compact ?: old.compact,
    ...
    fun setInnerPagingSpread(value: Boolean) { if (statePayloadInvalid) return; mutable.update { it.copy(innerPagingSpread = value) }; persist() }
    ...
    fun setIconShape(shape: IconShape) = setIconStyle(mutable.value.iconStyle.copy(shape = shape))
    fun setIconEffect(effect: IconEffect) = setIconStyle(mutable.value.iconStyle.copy(effect = effect))
    fun setIconPack(packageName: String?) = setIconStyle(mutable.value.iconStyle.copy(pack = packageName))
    fun setLiquidGlass(enabled: Boolean) = setIconStyle(mutable.value.iconStyle.copy(liquidGlass = enabled))
    fun setLiveIcons(enabled: Boolean) = setIconStyle(mutable.value.iconStyle.copy(liveIcons = enabled))
    /** Any style change re-renders every icon: the refresh keys its icon cache on the style. */
    fun setIconStyle(style: IconStyle) {
        if (mutable.value.iconStyle == style) return
        mutable.update { it.copy(iconStyle = style) }
        persist()
        refresh()
    }
    /** Drops every cached icon bitmap (for example after updating an icon pack) and re-renders. */
    fun refreshIcons() { iconRevision++; refresh() }
    ...
    internal fun restoreLayout(layout: HomeLayout) {
        ...
        mutable.update { it.copy(homeSlots = reconcileHomeSlots(layout.slots, installed),
            ...
            folders = layout.folders, widgetRestores = layout.widgetRestores, explicitPageCount = layout.explicitPageCount,
            hiddenPages = layout.hiddenPages, pairs = layout.pairs,
            canUndoEdit = false, editRevision = it.editRevision + 1) }
        undoLayout = null
        undoImportSettings = null
        persist()
    ...
    private fun persist() {
        ...
        val widgets = JSONArray().also { array -> s.widgetPlacements.forEach { w -> array.put(JSONObject()
            ...
            .put("spanX", w.spanX).put("spanY", w.spanY).putWidgetAppearance(w).putWidgetStack(w)) } }
        ...
        val pairs = JSONArray().also { array -> s.pairs.forEach { pair -> array.put(JSONObject()
        ...
        val data = JSONObject().put("schema", 13).put("pinned", JSONArray(s.order)).put("homeSlots", JSONArray(s.homeSlots))
            ...
            .put("pageCount", s.explicitPageCount)
            // B39 "Přehled stránek a režimy plochy".
            .put("hiddenPages", JSONArray(s.hiddenPages.sorted()))
            .put("modes", s.homeModes.toJsonArray())
            .put("activeMode", s.activeModeId)
            // "Stránkování inneru jako spread".
            .put("innerPagingSpread", s.innerPagingSpread)
            .put("compact", preset(s.compact)).put("expanded", preset(s.expanded))
            .putIconStyle(s.iconStyle)
        ...
        if (legacyRaw != null && sourceSchema < 11 && !prefs.contains("state_v10_backup"))
            editor.putString("state_v10_backup", legacyRaw)
        if (legacyRaw != null && sourceSchema < 12 && !prefs.contains("state_v11_backup"))
            editor.putString("state_v11_backup", legacyRaw)
        if (legacyRaw != null && sourceSchema < 13 && !prefs.contains("state_v12_backup"))
            editor.putString("state_v12_backup", legacyRaw)
    ...
    private fun load(): LauncherState = runCatching {
        ...
        require(schema <= 13) { "Unsupported saved-state schema $schema" }
        ...
        val rawLeadingSlots = if (schema >= 8) {
            ...
            if (schema >= 13) {
                ...
                // Schema 13 "Mřížka 4x7 a obsah výš" (2026-09-17 noc): GRID_ROWS was 6
                // (LEGACY_HOME_CELLS cells) when this array was written; rows 0-5 keep their
                // exact slot, the new row 6 starts empty.
                require(leading.length() == LEGACY_HOME_CELLS)
                migrateSchema12LeadingCells(List(LEGACY_HOME_CELLS) { leading.optString(it).takeIf { id -> id.isNotBlank() && id != "null" } })
            }
        ...
        val placements = if (schema >= 6) {
            ...
            List(widgetArray.length()) { index ->
                ...
                WidgetPlacement(strictInt(w, "slot"), strictInt(w, "id"), strictInt(w, "page"), strictInt(w, "column"), strictInt(w, "row"),
                    strictInt(w, "spanX"), strictInt(w, "spanY"), w.widgetAppearance(), w.widgetTint(),
                    w.widgetStackMembers(), w.widgetSmartRotate()).withSanitizedStack()
            }.also { loaded ->
                ...
                loaded.forEach { placement ->
                    ...
                    val migratedOverflow = placement.page > 0 && placement.slot / 3 == placement.page && placement.slot % 3 == 2 &&
                        placement.column == 0 && placement.row == LEGACY_FULL_WIDTH_OVERFLOW_ROW && placement.spanX == GRID_COLUMNS && placement.spanY == 4
        ...
        // B46 "Dvojice aplikací": same reserved-id shape and slot-membership checks as folders above.
        val pairs = if (schema >= 12) {
            val array = j.optJSONArray("pairs") ?: JSONArray()
            List(array.length()) { index ->
                val item = array.getJSONObject(index)
                PairEntry(item.getString("id"), item.getString("first"), item.getString("second"))
            }.also { loaded ->
                require(loaded.map(PairEntry::id).distinct().size == loaded.size)
                require(loaded.flatMap { listOf(it.first, it.second) }.distinct().size == loaded.size * 2)
                loaded.forEach { pair ->
                ...
                val members = loaded.flatMapTo(mutableSetOf()) { listOf(it.first, it.second) }
                val pairIds = loaded.mapTo(mutableSetOf(), PairEntry::id)
                val rawPairRefs = (rawSlots + rawLeadingSlots).filterNotNull().filter(::isReservedPairId)
                require(rawPairRefs.all(::isPairId))
                require(rawPairRefs.size == pairIds.size && rawPairRefs.toSet() == pairIds)
                require((rawSlots + rawLeadingSlots).none { it in members } &&
                    loadedDock.none { it in members || (it != null && isReservedPairId(it)) } &&
                    folders.none { folder -> folder.appIds.any { it in members } } &&
                    members.none { member -> folders.any { member in it.appIds } })
            }
        } else emptyList()
        if (schema >= 8) {
            ...
            val leadingApps = leadingIds.filterNot(::isReservedFolderId).filterNot(::isReservedPairId)
            val otherApps = rawSlots.filterNotNull().filterNot(::isReservedFolderId).filterNot(::isReservedPairId) +
                loadedDock.filterNotNull() + folders.flatMap(FolderEntry::appIds) + pairs.flatMap { listOf(it.first, it.second) }
        ...
        val loadedModes = if (schema >= 11) (j.optJSONArray("modes") ?: JSONArray()).toHomeModes() else defaultHomeModes()
        val loadedActiveMode = (if (schema >= 11) j.optString("activeMode", ALL_MODE_ID) else ALL_MODE_ID)
            .let { id -> if (loadedModes.any { mode -> mode.id == id }) id else ALL_MODE_ID }
        LauncherState(homeSlots = run {
                val schema5 = if (schema in 2..5) migrateSchema5Apps(legacySlots) else legacySlots
                // Schema 13 "Mřížka 4x7 a obsah výš": every earlier schema wrote 24-cell pages
                // (GRID_ROWS 6); widen each to 28, rows 0-5 unchanged, the new row 6 empty.
                if (schema < 13) migrateSchema12HomeCells(schema5) else schema5
            },
            leadingSlots = rawLeadingSlots,
            dock = loadedDock,
            widgetPlacements = placements, folders = folders, pairs = pairs, widgetRestores = restores,
            ...
            iconStyle = j.iconStyle(),
            // Absent (schema < 9, or never set) defaults to 1, which HomeLayout.pageCount always
            // folds into its derived minimum — an unmigrated layout keeps its existing page count.
            explicitPageCount = j.optInt("pageCount", 1).coerceAtLeast(1),
            // B39: absent (schema < 11) migrates to no hidden pages and just the built-in "Vše" mode.
            hiddenPages = if (schema >= 11) {
                val array = j.optJSONArray("hiddenPages") ?: JSONArray()
                List(array.length()) { array.optInt(it) }.toSet()
            } else emptySet(),
            homeModes = loadedModes, activeModeId = loadedActiveMode,
            // Absent (schema < 12) defaults to true: the new spread behaviour.
            innerPagingSpread = j.optBoolean("innerPagingSpread", true))
    }.getOrElse {
